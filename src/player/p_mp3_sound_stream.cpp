...
void MemoryDataCleanup(std::any* rawMp3Data);
...
mySamplingRate(0)
{
    ...
    mpg123_replace_reader_handle(myHandle, &MemoryDataRead, &MemoryDataLSeek, 
                                 reinterpret_cast<void (*)(void*)>(&MemoryDataCleanup));
}
...
ssize_t MemoryDataRead(void* rawMp3Data, void* buffer, size_t nbyte)
{
    ...
    else if (mp3Data->offset + (ssize_t)nbyte > (ssize_t)mp3Data->size)
    {
        ...
        memcpy(buffer, (unsigned char*)mp3Data->data + mp3Data->offset, readSize);
    ...
    {
        memcpy(buffer, (unsigned char*)mp3Data->data + mp3Data->offset, nbyte);
